...
func NewRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("cannot open \"%s\": %v", o.TemplateFile, err)
			}
			...
			if err != nil {
				return fmt.Errorf("cannot parse config %s: %v", o.TemplateFile, err)
			}
			...
			if err := os.WriteFile(o.OutputFile, res, 0o644); err != nil {
				return fmt.Errorf("cannot write to %s: %v", o.OutputFile, err)
			}
